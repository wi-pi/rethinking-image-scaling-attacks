"""
Reference:
https://github.com/cmhcbb/attackbox/blob/master/attack/Sign_OPT_v2.py
"""
import time
...
class OPT_attack_sign_SGD_v2(object):
    ...
    def sign_grad_v2(self, x0, y0, theta, initial_lbd, h=0.001, D=4, target=None):
        ...
        for iii in range(K):
            ...
            # Untargeted case
            preds.append(
                self.model.predict_label(x0 + torch.tensor(initial_lbd * new_theta, dtype=torch.float).cuda()).item())

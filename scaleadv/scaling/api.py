...
class ScalingAPI(object):
    ...
    def __init__(self, src_shape: ShapeType, tgt_shape: ShapeType,
        ...
        if isinstance(lib, str):
            ...
            if lib not in _str_to_lib:
                raise ValueError(f'Does not support lib {lib}.')
            lib = _str_to_lib[lib]
        if isinstance(alg, str):
            alg = alg.upper()
            if alg not in _str_to_alg:
                raise ValueError(f'Does not support alg {alg}.')
            alg = _str_to_alg[alg]
    ...
    def _check_input(self, x: np.ndarray):
        ...
        if x.shape[0] not in [1, 3]:
            raise ValueError(f'Only support 1/3 channels, but got {x.shape[0]} channels.')
        if not issubclass(x.dtype.type, np.floating):
            raise ValueError(f'Only support np.floating types, but got {x.dtype}.')
        if x.min() < 0 or x.max() > 1:
            raise Warning(f'Image should be normalized to [0, 1], but got [{x.min()}, {x.max()}].')
        if x.shape[1:] != self.src_shape:
            raise ValueError(f'Image shape does not match. Expect {self.src_shape}, but got {x.shape[1:]}')
        return x

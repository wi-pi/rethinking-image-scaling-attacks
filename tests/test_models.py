...
from src.models import imagenet_resnet50


def _get_imagenet_data_loader(batch_size, num_workers):
...
class TestImageNet(object):
    ...
    """
    batch_size = 1024
    loader = _get_imagenet_data_loader(batch_size=batch_size, num_workers=16)
    model_list = ['nature', 'robust-2']
    ...
    def test_accuracy(self, model: str, true_acc: float):
        ...
        logger.info(f'Start evaluating {model}.')
        test_acc = self._evaluate(classifier, attack=None, desc=f'Evaluate Model "{model}"')
    ...
    def test_robustness(self, model: str, true_rob: float):
        ...
        test_rob = self._evaluate(classifier, attack=attack, desc=f'Evaluate Model "{model}"')
    ...
    def _get_classifier(model: str) -> PyTorchClassifier:
        model = imagenet_resnet50(model).eval()
        model = DataParallel(model).cuda()
        classifier = PyTorchClassifier(model, nn.CrossEntropyLoss(), (3, 224, 224), 1000, clip_values=(0, 1))
        return classifier

    def _evaluate(self, classifier: PyTorchClassifier, attack: EvasionAttack | None, desc: str):
        acc = []
        for x, y in tqdm(self.loader, desc=desc):
